...
##############################################################################
    ...
    def _flow_stats_reply_handler(self, ev):
        domainHosts = ['0a:01:00:00:00:01', '0a:02:00:00:00:02',
                       '0b:01:00:00:00:01', '0b:02:00:00:00:02']
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            if SimpleMonitor.STATS_REPORT:                
                print("In Port %8x Eth Dst %17s Out Port %8x Bitrate %f" %
                      (in_port, eth_dst, out_port, rate))
            ...
            if rate > SimpleMonitor.ATTACK_THRESHOLD and self.ddos:
                print(
                    "*********************************More than threshold*******************************")
        ...
        victims = victims.intersection(
            {'0a:01:00:00:00:01', '0a:02:00:00:00:02'})
    ...
    def dealWithAttackers(self, victims):
        ...
        for victim in victims:
            ...
            print("Identified victim: MAC %s Host %s Switch %s Port %s" %
                  (victim, victimHost, victimSwitch, victimPort))
            print("--------Calling getAttacker function-----------")
            victimAttackers = self.getAttackers(victim)
            print("Attackers for victim %s: %s" %
                  (victimAttackers, victimHost))
    ...
    def delFlows(self, attacker):
        ...
        subprocess.call(["sudo", "ovs-vsctl", "del-port",
    ...
    def applyIngress(self):
        ...
        subprocess.call(["sudo", "ovs-vsctl", "set", "interface",
                        "s1-eth1", "ingress_policing_burst=100"])
        subprocess.call(["sudo", "ovs-vsctl", "set", "interface",
                        "s1-eth1", "ingress_policing_rate=40"])
